...
- (CGPoint)centerOfCellAtIndex:(NSUInteger)ind {
  if (![self checkIndex:ind]) return CGPointZero;
  NSUInteger row = ind / self.columnCount;
  NSUInteger col = ind % self.columnCount;
  return [self centerOfCellAtRow:row inColumn:col];
}

- (CGRect)frameOfCellAtIndex:(NSUInteger)ind {
  if (![self checkIndex:ind]) return CGRectZero;
  NSUInteger row = ind / self.columnCount;
  NSUInteger col = ind % self.columnCount;
  return [self frameOfCellAtRow:row inColumn:col];
}

- (BOOL)checkIndex:(NSUInteger)index{
  return index < self.columnCount * self.rowCount;
}
